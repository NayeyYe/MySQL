...
END //
DELIMITER ;
## 创建一个存储函数，删除在 Salary 表中有但在 Employees 表中不存在的员工号。若在 Employees 表中存在则返回 FALSE，若不存在则删除该员工号并返回 TRUE。
DELIMITER //
...
END //
DELIMITER ;
## 创建触发器，当修改 Employees 表时，若将 Employees 表中员工的工作时间增加 1 年，则将收入增加 500 元，若工作时间增加 2 年则收入增加 1000 元，依次增加。若工作时间减少则无变化。
DELIMITER //
